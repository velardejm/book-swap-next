...
export async function addBook(formData) {
  ...
  // });
  // const responseData = await response.json();
  const responseData = await postRequest("http://localhost:8000/api/dashboard/mybooks", requestData);
  if (responseData) {
...
export async function sendSwapRequest(requestedBook, offerredBook) {
  const { id: requestedBookId, owner_id: requesteeId, title: requestedBookTitle } =
    requestedBook;
  const { id: offerredBookId, owner_id: requesterId, title: offerredBookTitle } = offerredBook;
  const requestData = { requestedBookId, requestedBookTitle, requesteeId, offerredBookId, offerredBookTitle, requesterId }
  const responseData = await postRequest("http://localhost:8000/api/dashboard/requests", requestData);
  if (responseData) {
    // revalidateTag("mybooks");
    return true;
  }
...
export const postRequest = async (url, requestData, cacheType) => {
  const session = cookies().get("session")?.value;
  const options = {
    method: 'POST',
    cache: cacheType || "default",
